...
public class PopupWindowUtil {
    ...
    public static class SafeMode {
        private static final PopupWindowUtil mPopup = new PopupWindowUtil();
    }
    ...
    public void createScalePopupWindow(Context context, View inflate, View basicView, Double scaleX, Double scaleY) {
        int w = View.MeasureSpec.makeMeasureSpec(0, View.MeasureSpec.UNSPECIFIED);
        popupWindow = new PopupWindow(context);
        WindowManager wm = ((Activity) context).getWindowManager();
        int width = wm.getDefaultDisplay().getWidth();
        ...
        int scaleHeight = 0;
        if (scaleX == 0) {
            scaleWidth = basicView.getMeasuredWidth();
        } else if (scaleX == 1) {
            scaleWidth = width;
        } else {
            scaleWidth = (int) (width * scaleX);
        }

        if (scaleY == 0) {
            scaleHeight = ViewGroup.LayoutParams.WRAP_CONTENT;
        } else if (scaleY == 1) {
            scaleHeight = height;
        } else {
            scaleHeight = (int) (height * scaleY);
        }
